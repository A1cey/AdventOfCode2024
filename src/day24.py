...
def find_swap(gates: list[tuple[str, str, str, str]]) -> tuple[str, str] |None:
    ...
    for expr, output in expressions.items():
        ...
        if expr.position == -1 and output.startswith("z") and isinstance(expr, Mismatch):
            ...
            continue
        elif  isinstance(expr, GateExpression) and expr.label == "Digit" and output != f"z{expr.position:02d}":
...
def main():
    inputs, gates = parse_input(open("src/input/input24.txt"))
